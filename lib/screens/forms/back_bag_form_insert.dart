...
class BackBagFormScreen extends StatefulWidget {

  BackBagFormScreen({Key? key, required this.customer, required this.form, this.operation = 'add' }) : super(key: key);
  BackBagForm form;
  Customer customer ;
  String operation;
  @override
  _BackBagFormScreenState createState() => _BackBagFormScreenState();
}

class _BackBagFormScreenState extends State<BackBagFormScreen> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('الحقيبة الخلفية', style: TextStyle(color: txtColor)),
        centerTitle: true,
      ...
      body: Padding(
        ...
        child: ListView(
          children: [
            ...
            getLabelAndTextUI(context, "الواجهة الخلفية", "", widget.form.c1),
            getLabelAndTextUI(context, "ارضية الحقيبة", "", widget.form.c2),
            getLabelAndTextUI(context, "باب الحقيبة", "", widget.form.c3),
            getLabelAndTextUI(context, "فخذ خلفي ايمن", "", widget.form.c4),
            getLabelAndTextUI(context, "فخذ خلفي ايسر", "", widget.form.c5),
            getLabelAndTextUI(context, "هيكل خلفي ايمن", "", widget.form.c6),
            getLabelAndTextUI(context, "هيكل خلفي ايسر", "", widget.form.c7),
      ...
      child: ElevatedButton.icon(
        ...
        onPressed: () {
          if (widget.form.formNumber.text.trim() == '' ) {
            showErrorDialog(context, 'الرجاء ادخال رقم النموذج');
            return;
          }
          ...
          widget.form.saveDataToBackend(widget.customer.id, widget.operation).then((saved) => {
            ...
            if (saved) {
              showSuccessDialog(context, 'تم حفظ النموذج بنجاح !!'),
            }
            else {
              showErrorDialog(context, 'خطأ, اعد المحاولة لاحقا !!'),
            }
  ...
  void showSuccessDialog(BuildContext context, String info){
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
